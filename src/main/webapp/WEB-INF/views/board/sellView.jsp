...
<c:if test="${sessionScope.login != null}">
		...
		<div class="product-container">
			<c:forEach items="${sellList}" var="sell">
				<div class="product" onclick="window.location.href='<c:url value="/sellDetail?salesNo=${sell.salesNo}" />'">
					<img src="${pageContext.request.contextPath}${sell.salesImage }" alt="판매 이미지">
	...
	<div id="sell_stuff" class="container">
		<c:forEach items="${sellList}" var="sell">
			<div class="table-responsive" onclick="window.location.href='<c:url value="/sellDetail?salesNo=${sell.salesNo}" />'">
				<div class="sell-card" onclick="window.location.href='#'">
					<img src="${pageContext.request.contextPath}${sell.salesImage }" alt="Sell Image">
					<div class="sell-content">
						<h3>${sell.salesName }</h3>
						<p>판매자: ${sell.userNick }</p>
						<p>작성일: ${sell.salesDate }</p>
						<p class="sell-price">${sell.salesPrice }원</p>
						<p>거래 상태: ${sell.salesStatus }</p>
					</div>
				</div>
			</div>
		</c:forEach>
		...
		<nav aria-label="Page navigation example">
			<ul class="pagination justify-content-center">
				<c:if test="${searchVO.firstPage != 1}">
					<li class="page-item">
						<a class="page-link" href="sellView?curPage=${searchVO.firstPage-1}" data-page="${searchVO.firstPage-1}" aria-label="Previous">
				...
				<c:forEach begin="${searchVO.firstPage}" end="${searchVO.lastPage}" var="i">
					...
					<c:if test="${searchVO.curPage != i}">
						<li class="page-item">
							<a class="page-link" href="sellView?curPage=${i}" data-page="${i}">${i}</a>
						</li>
					</c:if>
				</c:forEach>
				<c:if test="${searchVO.lastPage != searchVO.totalPageCount}">
					<li class="page-item">
						<a class="page-link" href="sellView?curPage=${searchVO.lastPage+1}" data-page="${searchVO.lastPage+1}" aria-label="Next">
